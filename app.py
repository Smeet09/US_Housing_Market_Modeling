...
page = st.sidebar.radio("Go to", ["Home", "Housing vs Mortgage", "CPI vs Unemployment", "Income and GDP", "Housing Starts", "Correlation Heatmap", "Scatter Matrix", "Monthly Trends", "Yearly Boxplot", "GDP vs Housing", "Year-over-Year Trends"])
...
# PAGE: GDP vs Housing
    ...
    # PAGE: Year-over-Year Trends
elif page == "Year-over-Year Trends":
    st.title("📈 Year-over-Year Trends Analysis")

    # Calculate Year-over-Year percentage change
    df_yoy = filtered_df.copy()
    df_yoy["Home_Price_Index_YoY"] = df_yoy["Home_Price_Index"].pct_change(periods=12) * 100
    df_yoy["CPI_YoY"] = df_yoy["CPI"].pct_change(periods=12) * 100
    df_yoy["Real_GDP_YoY"] = df_yoy["Real_GDP"].pct_change(periods=12) * 100

    # Plot
    fig_yoy, ax = plt.subplots(figsize=(12, 5))
    df_yoy.set_index("observation_date")[["Home_Price_Index_YoY", "CPI_YoY", "Real_GDP_YoY"]].plot(ax=ax)
    ax.set_title("Year-over-Year Trends")
    ax.set_xlabel("Date")
